...
def main(config, operation):
    if config:
        ...
        deployment_type = config.get(consts.OPENSTACK).get(
            consts.DEPLOYMENT_TYPE)
        ...
        ansible_configuration.launch_provisioning_kolla(
            iplist, git_branch, hostname_map, host_node_type_map,
            docker_registry, docker_port, kolla_base, kolla_install, ext_sub,
            ext_gw, ip_pool_start, ip_pool_end, second_storage, operation,
            host_cpu_map, reserve_memory, base_size, count, default, vxlan)
...
def __hostip_list(config):
    ...
    # TODO/FIXME - using same variable 'i' in both inner and outer loops
    for i in range(len(hosts)):
        ...
        for i in range(len(interfaces)):
            ...
            intf_type = interfaces[i].get(consts.TYPE)
            if (intf_type == "management") and ip:
                host_ip = ip
...
def __create_global(config, git_branch):
    ...
    if (config.get(consts.OPENSTACK).get(consts.KOLLA).get(
        ...
        kolla_base_distro = config.get(consts.OPENSTACK).get(consts.KOLLA).get(
            consts.BASE_DISTRIBUTION)
        filedata = filedata.replace('#kolla_base_distro: "centos"',
                                    'kolla_base_distro: "'
                                    + kolla_base_distro + '"')
    ...
    if (config.get(consts.OPENSTACK).get(consts.KOLLA).get(
        ...
        filedata = filedata.replace('#kolla_install_type: "binary"',
                                    'kolla_install_type: "'
                                    + kolla_install_type + '"')
    ...
    if (config.get(consts.OPENSTACK).get(consts.KOLLA).get(
        ...
        filedata = filedata.replace('#keepalived_virtual_router_id: "51"',
    ...
    if (config.get(consts.OPENSTACK).get(consts.KOLLA).get(
        ...
        filedata = filedata.replace('#kolla_external_vip_address:',
    ...
    if (config.get(consts.OPENSTACK).get(consts.KOLLA).get(
        ...
        filedata = filedata.replace('#kolla_external_vip_interface:',
    ...
    if (config.get(consts.OPENSTACK).get(consts.KOLLA).get(
        ...
        docker_port = config.get(consts.OPENSTACK).get(consts.KOLLA).get(
            consts.KOLLA_REGISTRY_PORT)
        filedata = filedata.replace('#docker_registry: "172.16.0.10:4000"',
...
def __create_inventory_multinode(host_node_type_map):
    ...
    for key, value in host_node_type_map.items():
        ...
        if 'monitoring' in value:
            filedata = filedata.replace('[monitoring]',
                                        '[monitoring] \n' + key)
    shutil.copy2(basefile, newfile)
    f = open(newfile, 'w')
    f.write(filedata)
...
def __validate_configuration(config):
    ...
    if config_dict.get(consts.GIT_BRANCH) is not None:
        ...
        for key, value in host_dict.items():
            if key == "interfaces":
                ...
                for interfaces in value:
                    ...
                    if (interfaces.get(consts.IP) is None
                        ...
                        valid = False
                            ...
                            and interfaces.get(consts.TYPE).lower() !=
                            "tenant"):
                        logger.error("Invalid interface type")
                        valid = False

                    elif not interfaces.get(consts.NAME):
                        logger.error("Interface name must be defined")
                        valid = False
                    elif (interfaces.get(consts.IP)
                          and interfaces.get(consts.TYPE) == "management"):
    ...
    if ((config_dict.get(consts.KOLLA).get(
            consts.BASE_DISTRIBUTION) == 'ubuntu') or (
        config_dict.get(consts.KOLLA).get(
    ...
    if ((config_dict.get(consts.KOLLA).get(
        ...
        logger.info("NOT A VALID OPTION")
        valid = False
    if not config_dict.get(consts.KOLLA).get(
            consts.KEEPALIVED_VIRTUAL_ROUTER_ID):
        logger.info("VIRTUAL_ROUTER_ID CANNOT BE NULL")
        valid = False
    if not config_dict.get(consts.KOLLA).get(consts.INTERNAL_VIP_ADDRESS):
        logger.info("KOLLA_INTERNAL_VIP CANNOT BE NULL")
        valid = False
    if not config_dict.get(consts.KOLLA).get(consts.REGISTRY):
        logger.info("KOLLA_REGISTRY CANNOT BE NULL")
        valid = False
    if not config.get(consts.OPENSTACK).get(consts.KOLLA).get(
            "second_storage") and "ceph" in config.get(
