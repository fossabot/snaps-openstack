...
def __deploy_ansible_playbook_prepare(playbook_path, user, password,
                                      proxy_path, dpdk):
    """
    This method calls the preparation playbook for devstack which is
    responsible for all the installation and creation of Stack user
    ...
    os.system('pwd')
    os.system(
        '/usr/bin/ansible-playbook ' + playbook_path
        + ' --inventory=ansible_p/commission/openstack/playbooks/'
        + 'deploy_mode/devstack/temp.ini --extra-vars=\'{\"PROXY_PATH\": \"'
        + proxy_path + '\",\"dpdk\": \"' + dpdk + '\"}\'')
    os.chdir(cwd)
    return True
