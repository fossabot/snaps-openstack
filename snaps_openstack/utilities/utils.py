...
# TODO - Determine if this script is currently being used???
    ...
    for host in task.get("HOSTS"):
        ...
        for interface in host.get("interfaces"):
            ...
            if not size:
                ...
                # TODO/FIXME - Why is this return value not being evaluated?
        ...
        if args.tenant_vlan is not ARG_NOT_SET:
            for task in config.get("TASKS"):
                if task.get("name") == "TenantVLAN":
                    ...
                    # TODO/FIXME - why is ret getting set to 0 here???
        ...
        if args.mtu is not ARG_NOT_SET:
            for task in config.get("TASKS"):
                if task.get("name") == "mtu":
                    ...
                    # TODO/FIXME - why is this ret val not being evaluated???
                    ret = __mtu(task)
                    ret = 0
                    if ret == 0:
                        logger.info('Completed opeartion successfully')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-tvlan', '--tenant_vlan', dest='tenant_vlan',
