...
def main(arguments):
    ...
    print ("%s%s%s" % (
        '===========================', 'Current Exported Relevant Path',
        '=================================='))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--deploy', dest='deploy', nargs='?',
                        default=ARG_NOT_SET,
                        help='When used, deployment and provisioning of '
                             'openstack will be started')
    parser.add_argument('-c', '--clean', dest='clean', nargs='?',
                        default=ARG_NOT_SET,
                        help='When used, the openstack environment will be '
                             'removed')
    ...
    parser.add_argument('-drs', '--dreg', dest='dreg', nargs='?',
                        default=ARG_NOT_SET,
                        help='When used, to set up kolla registry along with'
                             ' deployment')
    ...
    if (args.dreg is ARG_NOT_SET and args.dregclean is ARG_NOT_SET and
            args.deploy is ARG_NOT_SET and args.clean is ARG_NOT_SET):
        logger.info('Must enter either -d for deploy IaaS or -c for cleaning'
                    ' up and environment or -drc to cleanup registry or -drs '
                    'to setup registry')
        exit(1)
    ...
    if args.deploy is not ARG_NOT_SET and args.config is ARG_NOT_SET:
        logger.info(
            'Cannot start deploy operation without configuration. Choose the'
            ' option -f/--file')
        exit(1)
